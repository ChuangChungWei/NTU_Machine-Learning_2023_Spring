...
class QA_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            ...
            bias=int(self.max_paragraph_len*ratio/2)
            bias=max(bias,(answer_end_token -answer_start_token)//2+1)
            
            #ratio=1

            paragraph_start = max(1, min(mid - bias, len(tokenized_paragraph) - self.max_paragraph_len))
            '''
...
def evaluate(data, output, doc_stride=200, paragraph=None, paragraph_tokenized=None):
    ##### TODO: Postprocessing #####
    # There is a bug and room for improvement in postprocessing 
    # Hint: Open your prediction file to see what is wrong 
...
#models=["hfl/chinese-pert-large","hfl/chinese-roberta-wwm-ext","luhua/chinese_pretrain_mrc_macbert_large"]
...
for i in range(len(models)):
    ...
    model_name=models[i]
    #model_name="saved_model/hfl/chinese-pert-large"
    ...
    train_questions, train_paragraphs = questions,paragraphs
    ...
    #scheduler = get_linear_schedule_with_warmup(optimizer,num_warmup_steps=5,num_training_steps=num_epoch*logging_step)
    ...
    for epoch in range(num_epoch):
        ...
        for data in tqdm(train_loader):	
            ...
            data[:5] = [i.to(device) for i in data[:5]]
            ...
            path = 'log/'+model_name.replace("/","_")+'.txt'
            f = open(path, 'a+')
                
            # Print training loss and accuracy over past logging step
            if step % logging_step == 0:
                print(f"Epoch {epoch + 1} | Step {step} | loss = {train_loss.item() / logging_step:.3f}, acc = {train_acc / logging_step:.3f}")
                print(f"Epoch {epoch + 1} | Step {step} | loss = {train_loss.item() / logging_step:.3f}, acc = {train_acc / logging_step:.3f}",file=f)
                train_loss = train_acc = 0
                '''
                print("========start==========",file=f)
                print("GT que/ans:",file=f)
                print(data[5]['question_text'][0],file=f)
                print(data[5]['answer_text'][0],file=f)
                p_id=data[5]['paragraph_id'][0].item()
                #print(p_id,type(p_id))
                print("Predict pargaraphs/ans:",file=f)
                print(paragraphs[p_id][int(data[6][0].item()):int(data[7][0].item())],file=f)
                #print(start_index)
                #print(paragraphs[p_id][start_index[0].item():end_index[0].item()],file=f)
                #print(paragraphs[p_id][data[3][0].item():data[4][0].item()],file=f)
                print(tokenizer.decode(data[0][0][start_index[0].item() : end_index[0].item() + 1]),file=f)
                print(tokenizer.decode(data[0][0][data[3][0].item() : data[4][0].item() + 1]),file=f)
                #print(evaluate(data, output))ddd
                print("=========end==========",file=f)
                '''
        ...
        if validation:
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(tqdm(dev_loader)):
                    ...
                    #print(evaluate(data, output) , dev_questions[i]["answer_text"])
                    dev_acc += evaluate(data, output) == dev_questions[i]["answer_text"]
                print(f"Validation | Epoch {epoch + 1} | acc = {dev_acc / len(dev_loader):.3f}")
                print(f"Validation | Epoch {epoch + 1} | acc = {dev_acc / len(dev_loader):.3f}",file=f)
            model.train()
    ...
    # Saved model can be re-loaded using 「model = BertForQuestionAnswering.from_pretrained("saved_model")」
        print("Saving Model ...")
        model_save_dir = "saved_model/"+model_name 
